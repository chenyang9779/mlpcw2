...
class AdamLearningRule(GradientDescentLearningRule):
    ...
    def __init__(self, learning_rate=1e-3, beta_1=0.9, beta_2=0.999, epsilon=1e-8):
        ...
        assert beta_1 >= 0.0 and beta_1 <= 1.0, "beta_1 should be in [0, 1]."
        assert beta_2 >= 0.0 and beta_2 <= 1.0, "beta_2 should be in [0, 2]."
        assert epsilon > 0.0, "epsilon should be > 0."
        self.beta_1 = beta_1
        self.beta_2 = beta_2
    ...
    def update_params(self, grads_wrt_params):
        ...
        for param, mom_1, mom_2, grad in zip(
            ...
            alpha_t = (
                self.learning_rate
                * (1.0 - self.beta_2 ** (self.step_count + 1)) ** 0.5
...
class AdaGradLearningRule(GradientDescentLearningRule):
    ...
    def update_params(self, grads_wrt_params):
        ...
        for param, sum_sq_grad, grad in zip(
            ...
            sum_sq_grad += grad**2
            param -= self.learning_rate * grad / (sum_sq_grad + self.epsilon) ** 0.5
...
class RMSPropLearningRule(GradientDescentLearningRule):
    ...
    def __init__(self, learning_rate=1e-3, beta=0.9, epsilon=1e-8):
        ...
        assert beta >= 0.0 and beta <= 1.0, "beta should be in [0, 1]."
        assert epsilon > 0.0, "epsilon should be > 0."
        self.beta = beta
        self.epsilon = epsilon
    ...
    def update_params(self, grads_wrt_params):
        ...
        Args:
            grads_wrt_params: A list of gradients of the scalar loss function
                ...
                previously, with this list expected to be in the same order.
        """
        for param, mom_2, grad in zip(self.params, self.moms_2, grads_wrt_params):
            mom_2 *= self.beta
            mom_2 += (1.0 - self.beta) * grad**2
            param -= self.learning_rate * grad / (mom_2 + self.epsilon) ** 0.5
