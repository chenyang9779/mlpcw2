...
class EMNISTPytorchDataProvider(Dataset):
    def __init__(
        ...
        self.numpy_data_provider = EMNISTDataProvider(
            which_set=which_set,
...
class Omniglot(data.Dataset):
    ...
    def __init__(
        self, root, set_name, transform=None, target_transform=None, download=False
    ):
        ...
        train_sample_idx = rng.choice(
            a=[i for i in range(len(x))], size=int(len(x) * 0.80), replace=False
        )
        evaluation_sample_idx = [i for i in range(len(x)) if i not in train_sample_idx]
        validation_sample_idx = rng.choice(
            a=[i for i in range(len(evaluation_sample_idx))],
            size=int(len(evaluation_sample_idx) * 0.40),
            replace=False,
        )
        test_sample_idx = [
            i
            for i in range(len(evaluation_sample_idx))
            if i not in evaluation_sample_idx
        ]
        ...
        if self.set_name == "train":
            ...
            self.data = [
                item for idx, item in enumerate(x) if idx in validation_sample_idx
            ]
...
class CIFAR10(data.Dataset):
    ...
    def __init__(
        ...
        train_sample_idx = rng.choice(
            a=[i for i in range(50000)], size=47500, replace=False
        )
