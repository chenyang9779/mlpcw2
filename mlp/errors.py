...
class BinaryCrossEntropyError(object):
    ...
    def __call__(self, outputs, targets):
        ...
        """
        return -np.mean(
            targets * np.log(outputs) + (1.0 - targets) * np.log(1.0 - outputs)
        )
    ...
    def grad(self, outputs, targets):
        ...
        """
        return (
            (1.0 - targets) / (1.0 - outputs) - (targets / outputs)
        ) / outputs.shape[0]
...
class BinaryCrossEntropySigmoidError(object):
    ...
    def __call__(self, outputs, targets):
        ...
        return -np.mean(targets * np.log(probs) + (1.0 - targets) * np.log(1.0 - probs))
